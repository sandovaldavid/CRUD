...
// Create a new row for each pet
const crearNuevaLinea = (nombre, edad, raza, dueno, id) => {
	...
	btn.addEventListener('click', () => {
		...
		modalTitle.textContent = 'Eliminar Mascota';
		modalText.textContent = '¿Estás seguro de que deseas eliminar esta mascota?';
		...
		const confirmDelete = async () => {
			try {
				await petServices.eliminarPet(id);
...
// Select the table where pets will be displayed
const table = document.querySelector('[data-table]');

// Load pets when the page loads
const cargarPets = async () => {
	try {
		const data = await petServices.listaPets();
		...
		data.forEach((pet) => {
			const { nombre, edad, raza, dueno, id } = pet;
			const nuevaLinea = crearNuevaLinea(nombre, edad, raza, dueno, id);
